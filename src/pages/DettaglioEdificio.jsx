      ...
      <section className="border-4 border-white p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                className="w-full bg-black border-2 border-white p-4 font-mono text-white uppercase focus:border-green-500 outline-none"
              />
            </div>
            <div>
              ...
              <input
                type="text"
                placeholder="ES. 001000..."
                value={codicePdr}
                onChange={(e) => setCodicePdr(e.target.value)}
                className="w-full bg-black border-2 border-white p-4 font-mono text-white uppercase focus:border-green-500 outline-none"
              />
            </div>
          </div>
          <button type="submit" className="bg-white text-black font-black px-6 py-3 uppercase hover:bg-green-500 w-full transition-none">
            Salva Dati Forniture
          </button>
        </form>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4 border-t-2 border-white">
            ...
            {edificio.pods && edificio.pods.length > 0 ? (
              edificio.pods.map((pod, idx) => (
                <div key={idx} className="border-2 border-white p-2 flex justify-between items-center font-mono text-sm mb-2">
                  {pod}{" "}
                  <button onClick={() => handleRimuoviPod(idx)} className="text-red-500 text-xs font-bold hover:text-white uppercase transition-none">
                    Rimuovi
                  </button>
                </div>
              ))
          ...
          <div>
            ...
            {edificio.pdr ? (
              <div className="border-2 border-green-500 p-2 font-mono text-sm text-green-500">{edificio.pdr}</div>
            ) : (
      ...
      {isCondominio && (
        <section className="border-4 border-white p-6">
          ...
          <form onSubmit={handleAggiungiUnita} className="space-y-4 mb-8">
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
              <div className="md:col-span-2">
                ...
                <input
                  ...
                  className="w-full bg-black border-2 border-white p-4 text-white uppercase focus:border-green-500 outline-none"
                />
              </div>
              <div>
                ...
                <input
                  type="text"
                  value={pianoUnita}
                  onChange={(e) => setPianoUnita(e.target.value)}
                  className="w-full bg-black border-2 border-white p-4 text-white uppercase focus:border-green-500 outline-none"
                />
              </div>
              <div>
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="w-1/2 bg-black border-2 border-white p-4 text-white uppercase focus:border-green-500 outline-none"
                  />
                  <input
                    type="text"
                    placeholder="PART"
            ...
            <button type="submit" className="w-full bg-white text-black font-black py-4 uppercase hover:bg-green-500 border-2 border-white transition-none">
              Registra Unità
            </button>
          </form>
          {unitaList.length > 0 && (
                ...
                <div key={u.id_unita} className="border-2 border-white p-4 flex justify-between items-center">
                  <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                    ...
                    <span className="text-green-500 font-bold uppercase">
                      SUB. {u.subalterno} | PIANO {u.piano}
                    </span>
                  </div>
                  <button onClick={() => handleRimuoviUnita(u.id_unita)} className="text-white hover:text-red-500">
                    <Trash2 size={24} />
                  </button>
                </div>
              ))}
      ...
      <section className="border-4 border-white p-6">
        ...
        <form onSubmit={handleCreaOAggiornaAmbiente} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div className="md:col-span-1">
              ...
              >
                ...
                {PIANI_STANDARD.map((p) => (
                  <option key={p} value={p}>
                    {p === "TERRA" ? "PIANO TERRA" : `PIANO ${p}`}
                  </option>
                ))}
            ...
            {isCustomPiano && (
              <div className="md:col-span-1">
                ...
                <input
                  ...
                  required
                  value={customPiano}
                  onChange={(e) => setCustomPiano(e.target.value)}
          ...
          <div className="flex gap-4">
            <button
              type="submit"
              className="flex-1 bg-white text-black font-black text-xl py-4 uppercase border-2 border-white hover:bg-green-500 transition-none"
            >
              {stanzaInModifica ? "Aggiorna Stanza" : "Crea Ambiente"}
            </button>
            {stanzaInModifica && (
              <button
                type="button"
                ...
                className="bg-black text-white border-4 border-white px-6 font-black uppercase text-sm hover:border-red-500 hover:text-red-500 transition-none"
              >
                Annulla
              </button>
            )}
      ...
      <section>
        ...
        <div className="grid gap-4">
          {ambienti?.map((ambiente) => (
            <div
              key={ambiente.id}
              className={`border-2 border-white p-6 flex flex-col sm:flex-row justify-between sm:items-center transition-none group gap-4 ${stanzaInModifica?.id === ambiente.id ? "border-green-500 bg-green-950" : "hover:border-green-500"}`}
            >
              ...
              <div className="flex items-center gap-2 justify-end border-t-2 border-dashed sm:border-0 border-zinc-800 pt-3 sm:pt-0">
                <button onClick={(e) => caricaModificaStanza(e, ambiente)} className="text-white hover:text-green-500 p-2 border border-white transition-none">
                  <Edit2 size={20} />
                </button>
                <button
                  onClick={(e) => handleRimuoviAmbiente(e, ambiente.id)}
                  className="text-white hover:text-red-500 p-2 border border-white hover:border-red-500 transition-none"
                >
                  <Trash2 size={20} />
                </button>
                <Link
                  to={`/ambiente/${ambiente.id}`}
                  className="font-black text-sm bg-white text-black px-4 py-2 uppercase hover:bg-green-500 border-2 border-white hover:border-green-500 transition-none"
                >
                  Apri
                </Link>
              </div>
